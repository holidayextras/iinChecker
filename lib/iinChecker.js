...
( function() {
  ...
  var defaultOptions = require( '../configs/options' );
  var cache = require( './cache' );

  var iinChecker = function( inOptions ) {
    // if there's any overriding options, blat them over the defaults
    this.options = _.extend( {}, defaultOptions, inOptions );
    if ( this.options.cache ) {
      // initialise cache
      cache.start( this.options.cacheServerConfig, this.options.cachePolicy );
    }
      ...
      request( { url: providerDetails.domain + providerDetails.path + iin, json: true }, function( error, response, body ) {
        if ( !error && response.statusCode === 200 ) {
          ...
          if ( self.options.cache &&
            cardInfo.brand && cardInfo.brand !== self.brands.UNKNOWN &&
            cardInfo.type && cardInfo.type !== self.nullValue ) {
            cache.set( iin, cardInfo, self.options.cachePolicy );
          }
  ...
  iinChecker.prototype.lookup = function( iin, callback ) {
    try {
      ...
      if ( self.options.cache ) {
        cache.get( iin, self.options.cachePolicy )
        .then( function( data ) {
