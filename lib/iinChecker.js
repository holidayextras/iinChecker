...
( function() {
  ...
  var iinChecker = function( iinOptions ) {
    ...
    this.isCacheable = function( cardInfo ) {
      return (this.options.cache && cardInfo.brand && cardInfo.brand !== this.brands.UNKNOWN && cardInfo.type && cardInfo.type !== this.nullValue);
    };
  ...
  iinChecker.prototype.lookup = function( iin, callback ) {
    ...
    function bindListeners( listenerCallback ) {
      ...
      function stopListening() {
        emitter.removeAllListeners('requestStarted')
          .removeAllListeners('requestComplete')
          .removeAllListeners('requestFailed')
          .removeAllListeners('regexResult')
          .removeAllListeners('returnError');
      }

      function clearRequestQueue() {
        _.forEach(currentRequests, function( currentRequest ) {
          // cancel the request - we don't want any long running requests hanging about
          currentRequest.abort();
        });
      }

      // listen to the "requestStarted" event - here we add requests into our "currentRequests" queue so we can keep track of them
      emitter.on('requestStarted', function( theRequest, iinToLookup ) {
        if ( iinToLookup === iin ) {
